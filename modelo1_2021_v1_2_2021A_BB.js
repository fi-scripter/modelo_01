/*< 455 > ══════════════════════════════ Números - Exploração - Banco de Dados ══════════════════════════════
	** MODELO 1 **
...
function criaDB(z,db){
	...
	if(db=="exploração" || escolha_db=="default"){
		...
		EscolhaProficiencia_menusStatus = [
			...
			/*005*/ "Quantos Lugares diferentes conheceu."
					...
					//Aqui é aonde virá as descrições para janela de descrição do status
					function retornainfo(p,k){
					var resssp="";
					if(k==1){
					//variavel[x][y]=z, atentar para que haja a correta indicação da posição no evento do maker
					if(p==1){resssp = "\nModelo 1 on\n";}
					if(p==2){resssp = "\nModelo 1 é infinito\n";}
					}
					//---------------------------------------------------------//
	...
	}else if(db=="numeros"){			//SELECIONAR LINHA AQUI ATE
		// Item a escolher		
		EscolhaProficiencia_menusOP = [
	...
	EscolhaProficiencia_db = [
			...
			/*002*/	"...ok",    //Nao está funcionando usar função dentro dessa parte do db com alert.
			/*003*/	"",
			/*004*/ "",
			/*005*/ ""
		];
